...
void print_chip_MACS()
{
  ...
  ESP_LOGI("Chip Info", "ESP_MAC_WIFI_STA: %s",mac_str);
  
  esp_read_mac(mac, esp_mac_type_t::ESP_MAC_WIFI_SOFTAP);
  sprintf(mac_str, "%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
  ESP_LOGI("Chip Info", "ESP_MAC_WIFI_SOFTAP: %s",mac_str);

  esp_read_mac(mac, esp_mac_type_t::ESP_MAC_BT);
  sprintf(mac_str, "%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
  ESP_LOGI("Chip Info", "ESP_MAC_BT: %s",mac_str);

  esp_read_mac(mac, esp_mac_type_t::ESP_MAC_ETH);
  sprintf(mac_str, "%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
  ESP_LOGI("Chip Info", "ESP_MAC_ETH: %s",mac_str);

  //esp_read_mac(mac, esp_mac_type_t::ESP_MAC_IEEE802154);
  //sprintf(mac_str, "%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
  //ESP_LOGI("Chip Info", "ESP_MAC_IEEE802154: %s",mac_str);	  

  esp_read_mac(mac, esp_mac_type_t:: ESP_MAC_BASE);
  sprintf(mac_str, "%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
  ESP_LOGI("Chip Info", "ESP_MAC_BASE: %s",mac_str);

  esp_read_mac(mac, esp_mac_type_t::ESP_MAC_EFUSE_FACTORY);
  sprintf(mac_str, "%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
  ESP_LOGI("Chip Info", "ESP_MAC_EFUSE_FACTORY: %s",mac_str);

  //esp_read_mac(mac, esp_mac_type_t::ESP_MAC_EFUSE_CUSTOM);
  //sprintf(mac_str, "%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
  //ESP_LOGI("Chip Info", "ESP_MAC_EFUSE_CUSTOM: %s",mac_str);
  
  //esp_read_mac(mac, esp_mac_type_t::ESP_MAC_EFUSE_EXT);
  //sprintf(mac_str, "%02X:%02X:%02X:%02X:%02X:%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
  //ESP_LOGI("Chip Info", "ESP_MAC_EFUSE_EXT: %s",mac_str);
}
...
void print_memory_sizes(void) {
    ...
    ESP_LOGI("Memory Info", "Total DRAM (internal memory): %ld bytes", total_internal_memory);
    ESP_LOGI("Memory Info", "Free DRAM (internal memory): %ld bytes", free_internal_memory);
    ESP_LOGI("Memory Info", "Largest free contiguous DRAM block: %ld bytes", largest_contig_internal_block);
